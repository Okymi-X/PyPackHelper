...
class PackageUploader:
        ...
        try:
            ...
            if (self.package_path / "setup.py").exists():
                try:
                    ...
                    if result.returncode != 0:
                        print(f"❌ Failed to build source distribution: {result.stderr}")
                        return False
    ...
    def upload_package(self, test: bool = False) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if package_name:
                    if test:
                        print(f"🔧 Test installation: pip install -i https://test.pypi.org/simple/ {package_name}")
                    else:
            ...
            try:
                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    if wheel_files:
                        ...
                        result = subprocess.run(
                            ["pip", "install", "--target", str(temp_path), str(wheel_file)],
                            capture_output=True,
                            text=True
                        )
                        ...
                        if result.returncode == 0:
                            ...
                            print(f"⚠️  Package installation test failed: {result.stderr}")
                            return False
    ...
    def _get_credentials(self, test: bool = False) -> tuple:
        ...
        if test:
            ...
            password = os.getenv("TESTPYPI_TOKEN", os.getenv("TESTPYPI_PASSWORD", os.getenv("TWINE_PASSWORD")))
        else:
            username = os.getenv("PYPI_USERNAME", os.getenv("TWINE_USERNAME"))
            password = os.getenv("PYPI_TOKEN", os.getenv("PYPI_PASSWORD", os.getenv("TWINE_PASSWORD")))
    ...
    def get_upload_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "build_available": self._check_command_available("python") and self._check_build_available()
        }
