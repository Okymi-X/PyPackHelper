...
def init(
    ...
    license_type: Optional[str] = typer.Option("MIT", "--license", "-l", help="License type"),
    github_actions: bool = typer.Option(True, "--github-actions/--no-github-actions", help="Generate GitHub Actions workflow")
):
...
@app.command()
def validate(
    path: Optional[str] = typer.Option(None, "--path", "-p", help="Package directory (default: current)"),
    lint_only: bool = typer.Option(False, "--lint-only", help="Run only linting checks"),
...
@app.command()
def bump(
    version_type: str = typer.Argument(..., help="Version type: major, minor, or patch"),
    path: Optional[str] = typer.Option(None, "--path", "-p", help="Package directory (default: current)"),
...
@app.command()
def upload(
    test: bool = typer.Option(False, "--test", help="Upload to TestPyPI instead of PyPI"),
    path: Optional[str] = typer.Option(None, "--path", "-p", help="Package directory (default: current)"),
    skip_validation: bool = typer.Option(False, "--skip-validation", help="Skip validation before upload"),
    skip_build: bool = typer.Option(False, "--skip-build", help="Skip building package"),
    clean: bool = typer.Option(True, "--clean/--no-clean", help="Clean build directories after upload")
):
    ...
    try:
        ...
        if not skip_validation:
            ...
            if not validator.validate_all():
                typer.echo("❌ Validation failed. Use --skip-validation to force upload.", err=True)
                sys.exit(1)
...
def build(
    path: Optional[str] = typer.Option(None, "--path", "-p", help="Package directory (default: current)"),
    clean: bool = typer.Option(True, "--clean/--no-clean", help="Clean build directories before building")
):
